...
if __name__ == "__main__":
    ...
    pts = np.loadtxt("rehden_test.txt")
    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(pts)
    pcd_clean, _ = pcd.remove_statistical_outlier(nb_neighbors=100, std_ratio=5)
    pts = np.asarray(pcd_clean.points)

    sTag2 = builder.build_from_points(pts)

    # Keep original points visible
    for x, y, z in pts:
        gmsh.model.occ.addPoint(float(x), float(y), float(z), 0.0)

    gmsh.model.occ.synchronize()

    builder.write_step("rehden_test.step")
