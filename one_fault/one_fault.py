...
gmsh.model.occ.addBox(0, 0, 0, 2000,2000,750,1)
...
for idx, extracted_points in enumerate(back_transformed_vertices_list):
    ...
    # Stack x and y into 2D point coordinates
    points = np.column_stack((x, y))
    values = z

    # Fit RBFInterpolator with smoothing
    rbf_model = RBFInterpolator(points, values, kernel='linear', smoothing=1.0)  # adjust smoothing here
    rbf_models.append(rbf_model)

    # Evaluate RBF on a regular grid
    grid_x, grid_y = np.meshgrid(np.linspace(0, 2000, grid_res),
                                 np.linspace(0, 2000, grid_res))
    grid_points = np.column_stack((grid_x.ravel(), grid_y.ravel()))
    grid_z = rbf_model(grid_points).reshape(grid_x.shape)


    
    # Flatten grid to point list
    points = np.column_stack((grid_x.flatten(), grid_y.flatten(), grid_z.flatten()))

    # Create list to store point tags
    point_tags = []
    ...
    # Synchronize before adding surfaces
...
gmsh.model.occ.fragment(all_surfaces,[(3,1)], -1, True, True)
gmsh.model.occ.synchronize()
...
volumes = gmsh.model.getEntities(3)  # Get all volume entities

# Dictionary to collect tags for each physical group name
layer_groups = {}
tolerance = grid_res  # Tolerance for boundary matching

for (dim, tag) in volumes:
    elemTypes, elemTagsList, nodeTagsList = gmsh.model.mesh.getElements(dim, tag)

    try:
        idx = list(elemTypes).index(4)  # 4 = tetrahedral
    except ValueError:
        print(f"No tetrahedral elements found in volume {tag}")
        continue

    elemTags = elemTagsList[idx]
    nodeTags = nodeTagsList[idx]
    num_nodes_per_elem = 4

    if len(nodeTags) < num_nodes_per_elem:
        print(f"Not enough nodes in volume {tag}")
        continue

    num_elems = len(nodeTags) // num_nodes_per_elem
    if num_elems < 2:
        continue

    random_elem_idxs = random.sample(range(num_elems), 10)
    volume_matched_layer = None

    for i in range(1, len(rbf_models) - 1):  # Iterate over possible layers
        z1_rbf = rbf_models[i]
        z2_rbf = rbf_models[i + 1]
        all_elements_match = True

        for elem_idx in random_elem_idxs:
            elem_node_tags = nodeTags[elem_idx * num_nodes_per_elem : (elem_idx + 1) * num_nodes_per_elem]
            coords = []

            for node_tag in elem_node_tags:
                coord, _, _, _ = gmsh.model.mesh.getNode(node_tag)
                coords.append(coord)

            coords = np.array(coords).reshape(-1, 3)  # 4 x (x,y,z)
            ...
            for node_coord in coords:
                ...
                # Strict match with tolerance near boundaries
                if z < z_min:
                    ...
                    if abs(z - z_max) > tolerance:
                        all_elements_match = False
                        break
                # else: z is within bounds — OK

            if not all_elements_match:
                break  # One element failed; skip to next layer

        if all_elements_match:
            volume_matched_layer = f"Layer_{i}"
            break  # Found the first layer match

    # Assign the appropriate tag
    if volume_matched_layer:
        layer_groups.setdefault(volume_matched_layer, []).append(tag)
    else:
        layer_groups.setdefault("overburden", []).append(tag)

# Register physical groups
for name, tags in layer_groups.items():
